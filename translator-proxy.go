...
var (
	...
	apiUrl   = flag.String("apiUrl", "https://translate.api.cloud.yandex.net/translate/v2/translate", "Yandex Translate API URL")
	apiToken = flag.String("apiToken", "", "IAM token; must be set")
)
...
func (h *Handler) Options(response http.ResponseWriter, request *http.Request) {
	h.Default(response, request)
}
